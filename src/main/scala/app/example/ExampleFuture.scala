...
//    val futureSec: Future[Double] = futureMilliSec.map(i => i.toDouble / 1000)
    val futureSec: Future[Double] = futureMilliSec.flatMap(i =>
      Future {
        Thread.sleep(100)
        i.toDouble / 1000
    })
...
object PromiseSample {
  def run() = {
    ...
    val futureByPromise
      : Future[Int] = promiseGetInt.future // PromiseからFutureを作ることが出来る
...
class CallbackSomething {
  ...
  def doSomething(onSuccess: Int => Unit,
    ...
    if (i < 20) onSuccess(i) else onFailure(new RuntimeException(i.toString))
  }
}
...
class FutureSomething {
  ...
  def doSomething(): Future[Int] = {
    ...
    callbackSomething.doSomething(i => promise.success(i),
                                  t => promise.failure(t))
    promise.future
  }
}
...
object ExampleFuture {
  def executeSample1() = {
    println("[ExampleFuture:executeSample1]------------------")

    println("------------------")
    FutureSample.run()

    println("------------------")
    FutureOptionUsageSample.run()

    println("------------------")
    CompositeFutureSample.run()
